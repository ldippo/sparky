...
const Carousel = React.forwardRef<any, any>(function Carousel(
  { children, curPage, onChangeActive },
  ref
) {
  ...
  const paginate = useCallback((count: number) => {
    setPage([page + count, count]);
  }, [page]);

  const changeSlide = React.useCallback((count: number) => {
    paginate(count);
    if (onChangeActive) onChangeActive(count);
  },
